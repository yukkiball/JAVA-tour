...
121. 当某个类变量使用了final修饰，而且它的值在编译时就确定下来，那么程序在其他地方使用该变量时，实际上用常量代替了变量，所以不会初始化该类

122. 使用ClassLoader类的loadClass方法来加载某个类时，该方法只是加载该类，不会执行初始化，而Class的forName()方法会导致该类的强制初始化

123. Java中，一个类用其全限定类名（包括包名和类名）作为标识，而在JVM中，一个类用全限定类名和其类加载器作为唯一标识

124. 类加载器的机制：全盘负责、父类委托、缓存机制

125. JVM的根类加载器不是Java实现的，因此访问扩展类加载器的父类加载器时返回Null
